# Commons web product contracts

Durable owner decisions, organized by the area being changed. Read the relevant
sections through web/AGENTS.md. These constraints remain in force; this file
does not grant deployment or runtime authorization.
...
- 2026-08-13 superseding identity decision: the user rejected the generic `Link` icon and selected a dedicated original Commons identity grounded in the supplied motion reference and Option 2 direction. Use the two generated transparent RGBA connecting/resolved keyframes, with wrapper/image-only animation, a fully static resolved presentation under `prefers-reduced-motion`, and the resolved keyframe as the favicon. Preserve sanitized provenance and exact checksums. Do not use or approximate an official OpenAI, Codex, Blossom, Grok/X, or other protected mark.
