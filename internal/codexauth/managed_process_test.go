...
func (p *managedProcessTestDouble) LaunchHistorianTask(context.Context, string, string, string, string, string, string, DynamicToolHandler, TurnTerminalHandler) (TaskLaunch, error) {
	p.mu.Lock()
	p.launchCalls++
	err := p.launchErr
	p.mu.Unlock()
...
func TestManagedUncertainMutationsDoNotRetry(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			call: func(m *ManagedProcessClient, ctx context.Context) error {
				_, err := m.LaunchHistorianTask(ctx, "/tmp/project", "model", "max", "prompt", "message", "title", nil, nil)
				return err
			},
			count:        func(p *managedProcessTestDouble) int { _, calls, _ := processCounter(t, p); return calls },
			experimental: true,
