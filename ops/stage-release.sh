...
: "${COMMONS_OPS_SOURCE:?COMMONS_OPS_SOURCE required}"
...
ops_source=$COMMONS_OPS_SOURCE
case "$ops_source" in /*) ;; *) exit 64;; esac
...
test -f "$ops_source"; test ! -L "$ops_source"; test -x "$ops_source"
test "$($ops_source --build-id)" = "$id"
go version -m "$ops_source" | grep -Fq 'path'"$(printf '\t')"'codex-commons/cmd/commons-ops'
go version -m "$ops_source" | grep -Fq 'build'"$(printf '\t')"'-trimpath=true'
...
cp "$ops_source" "$target/commons-ops"
...
for runtime_op in backup.sh check-readiness.sh deploy-release.sh record-evidence.sh restore-database.sh seal-archive.sh verify-release.sh verify-restore.sh; do cp "ops/$runtime_op" "$target/ops/$runtime_op"; done
chmod 0755 "$target/commons-server" "$target/commons-ops" "$target/bin/codex" "$target/bin/codex-code-mode-host" "$target/codex-resources/bwrap" "$target/codex-resources/zsh/bin/zsh" "$target/codex-path/rg"
...
chmod 0555 "$target/commons-server" "$target/commons-ops" "$target/bin/codex" "$target/bin/codex-code-mode-host" "$target/codex-resources/bwrap" "$target/codex-resources/zsh/bin/zsh" "$target/codex-path/rg" || exit 64
