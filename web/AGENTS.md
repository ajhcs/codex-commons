...
- Commons' original identity is the raster companion in `src/assets/identity/commons-companion.png`: an asymmetric pearl-ceramic/brushed-metal convergence artifact with a small blue signal core. Use `CommonsCompanion.jsx` and animate its wrapper/state transitions; do not redraw it with CSS, divs, inline SVG, or handcrafted vector art.
- Companion motion is state-driven (`idle`, `connecting`, `authorized`, `identity-resolved`, `history-offered`), GPU-friendly, and must become fully static under `prefers-reduced-motion`.
- OpenAI's public brand page is design-language influence only: whitespace, circular geometry, confident hierarchy, monochrome restraint, and precise blue states. Never bundle or imitate OpenAI Sans, the Blossom, the OpenAI wordmark, OpenAI product marks, Grok/X marks, or other protected brand assets.
